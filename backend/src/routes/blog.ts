...
blogRouter.get('/:slug', async (req: Request, res: Response) => {
  ...
  if (!post.isPublished) return res.status(404).json({ error: 'Post not found' })
  res.json({ post })
})

blogRouter.post('/', requireAuth, requireTeamMember, auditLog('BlogPost'), async (req: AuthRequest, res: Response) => {
...
blogRouter.put('/:id', requireAuth, requireTeamMember, auditLog('BlogPost'), async (req: AuthRequest, res: Response) => {
...
blogRouter.delete('/:id', requireAuth, requireAdmin, auditLog('BlogPost'), async (req: Request, res: Response) => {
