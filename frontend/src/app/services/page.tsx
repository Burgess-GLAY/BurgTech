    ...
    <div className="pt-28 pb-24">
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        <div className="text-center mb-16">
          <span className="text-cyan-400 text-sm font-medium tracking-wide uppercase">Services</span>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((s: any, i: number) => (
            <motion.div key={s.id} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }}>
              <Link href={`/services/${s.slug}`} className="group flex flex-col gap-4 p-6 glass-card hover:border-cyan-400/20 transition-all h-full">
                <h3 className="font-semibold text-lg group-hover:text-cyan-400 transition-colors">{s.title}</h3>
                ...
                <div className="flex items-center gap-1 text-xs text-white/30 group-hover:text-cyan-400 transition-colors">
