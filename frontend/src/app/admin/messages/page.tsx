...
export default function AdminMessages() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => apiClient.get(`/messages${filter !== 'ALL' ? `?status=${filter}` : ''}`).then(r => r.data),
  })
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    {msg.status === 'UNREAD' && <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 flex-shrink-0" />}
                  </div>
