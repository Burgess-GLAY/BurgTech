...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#080c18] flex overflow-hidden">
      {/* Backdrop */}
      <AnimatePresence>
        {isSidebarOpen && (
          <motion.div
      ...
      {/* Sidebar */}
      <aside className={cn(
        "fixed inset-y-0 left-0 w-64 bg-[#080c18] border-r border-white/[0.06] flex flex-col z-50 transition-transform duration-300 transform lg:translate-x-0 lg:static lg:flex-shrink-0",
          ...
          {NAV.filter(n => !n.superOnly || user.role === 'SUPER_ADMIN').map(item => {
            ...
            return (
              <Link key={item.href} href={item.href} className={cn(
                ...
                active ? 'text-cyan-400 font-semibold' : 'text-white/40 hover:text-white hover:bg-white/[0.03]'
              )}>
                {active && <motion.div layoutId="nav-bg" className="absolute inset-0 bg-cyan-400/10 rounded-xl" />}
                <Icon className={cn("w-4 h-4 flex-shrink-0 relative z-10", active ? "text-cyan-400" : "text-white/30 group-hover:text-white")} />
        ...
        <div className="px-4 py-6 border-t border-white/[0.06] bg-white/[0.01]">
          <div className="flex items-center gap-3 px-2 mb-4">
            <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-blue-500/20 to-cyan-400/20 flex items-center justify-center text-xs font-bold text-cyan-400 border border-cyan-400/20">{user.name.charAt(0)}</div>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        {/* Header */}
        <header className="h-16 border-b border-white/[0.06] bg-[#080c18]/80 backdrop-blur-md flex items-center justify-between px-4 lg:px-8 flex-shrink-0 z-30">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/" target="_blank" className="text-[11px] font-bold uppercase tracking-widest px-4 py-2 rounded-xl border border-white/10 text-white/40 hover:text-white hover:border-cyan-400/30 hover:bg-cyan-400/5 transition-all duration-300">
              View site
            </Link>
          </div>
        </header>
