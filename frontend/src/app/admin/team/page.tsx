...
import { Plus, Edit2, Trash2, X, Check, Save, Users } from 'lucide-react'
    ...
    <>
      <td className="px-6 py-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-slate-800 overflow-hidden flex items-center justify-center text-white/40 shrink-0">
            {member.photoUrl ? (
      ...
      <td className="px-6 py-4">
        <span className={cn(
          ...
          member.isVisible ? 'bg-bt-cyan-subtle text-bt-cyan' : 'bg-white/5 text-white/40'
        )}>
      ...
      <td className="px-6 py-4 text-right">
        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <button onClick={() => handleDelete(member.id)} className="p-2 rounded-lg text-red-400 hover:bg-red-500/10 transition-all">
    ...
    <div className="glass-card p-4 flex flex-col gap-3">
      ...
      <div className="flex items-center gap-3">
        {member.photoUrl ? (
          ...
          <div className="w-11 h-11 rounded-xl bg-bt-teal flex items-center justify-center text-white font-bold text-base flex-shrink-0">
            {member.user?.name?.charAt(0) || '?'}
          </div>
        )}
        <div className="min-w-0">
          <p className="text-sm font-semibold text-white truncate">{member.user?.name || 'Team Member'}</p>
          <p className="text-xs text-bt-cyan truncate">{member.title}</p>
        </div>
      </div>
        ...
        <div className="text-right">
          ...
          <button
            ...
            className={cn(
              ...
              member.isVisible ? 'bg-bt-cyan-subtle text-bt-cyan border-bt-cyan-border' : 'bg-white/[0.05] text-white/40 border-white/[0.08]'
            )}
          >
  ...
  return (
    <div className="space-y-8">
      ...
      {loading ? (
        ...
        <ListTable<TeamMember>
          items={members}
          searchKeys={['title', 'bio']}
          filters={FILTERS}
          columns={COLUMNS}
          renderRow={renderRow}
          renderCard={renderCard}
          storageKey="admin-team-view"
          emptyState={
            <EmptyState
              icon={Users}
              title="No team members"
              description="Start by adding your first team member to showcase your expertise."
